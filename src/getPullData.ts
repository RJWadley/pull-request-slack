...
// biome-ignore lint/suspicious/noExplicitAny: octokits types are a royal fucking pain so idc
...
export const getPullData = async (): Promise<MappedPull[]> => {
	...
	for (const repo of repositories) {
		 ...
		 */
		const newData = await octokit.request(PULL_QUERY, {
			owner: repo.split("/")[0],
			repo: repo.split("/")[1],
			state: "all",
			per_page: 100,
		...
		for (const pull of pulls) {
			 ...
			 */
			const checkState: "pending" | "passing" | "failing" =
				pull.state !== "open"
					? "pending"
			...
			mappedData.push({
				...
				number: pull.number ?? Number.POSITIVE_INFINITY,
				link: pull.html_url,
